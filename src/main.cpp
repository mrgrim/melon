...
//    {
...
#if NBT_DEBUG == true
    parsed_nbt = new nbt::compound(std::move(nbt_data_ptr), nbt_data_size, pmr_rsrc);
#else
    for (int index = 0; index < 50000; index++)
    {
    ...
    auto level_compound = parsed_nbt->get<nbt::tag_compound>("Data").value();
    ...
    auto my_compound = new nbt::compound("");
